  ...
  describe('/api/articles/:article_id/comments', () => {
    ...
    describe('/api/articles/:article_id/comments/:comment_id', () => {
      describe('PATCH', () => {
        ...
        it('returns 400 for invalid parametric', () => request
          .patch('/api/articles/Madonna/comments/1')
          .send({ inc_votes: 3 })
          .expect(400)
          .then(({ body }) => {
            expect(body).to.eql({
        ...
        it('returns 404 for valdid parametric with invalid path', () => request
          .patch('/api/articles/12345/comments/1')
          .send({ inc_votes: 3 })
          .expect(404)
          .then(({ body }) => {
            expect(body).to.eql({ msg: 'silly! no pages here' });
      ...
      describe('DELETE', () => {
        it('returns 400 for invalid article_id parametric', () => request
          .delete('/api/articles/Madonna/comments/1')
          .expect(400)
          .then(({ body }) => {
            expect(body).to.eql({
              msg: 'invalid input syntax for data type',
        ...
        it('returns 400 for invalid comment_id parametric', () => request
          .delete('/api/articles/1/comments/Madonna')
          .expect(400)
          .then(({ body }) => {
            expect(body).to.eql({
              msg: 'invalid input syntax for data type',
        ...
        it('returns 404 for valdid parametric with invalid path', () => request
          .delete('/api/articles/12345/comments/1')
          .expect(404)
          .then(({ body }) => {
            expect(body).to.eql({ msg: 'silly! no pages here' });
          }));
        ...
        it('incorrect METHOD returns 405 and error message', () => {
          ...
          return Promise.all(
            invalidMethods.map(method => request[method]('/api/articles/1/comments/1')
              .expect(405)
              .then(({ body }) => {
                expect(body.msg).to.equal('method not allowed');
              })),
